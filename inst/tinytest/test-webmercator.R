...
# Check inputs
expect_error(
  sgo_wgs84_en(sgo_points(
    list(3487823.234, -305433.201, 5313739.634),
    epsg = 4936
  )),
  "only supports WGS84 or ETRS89 polar entries"
)
expect_error(
  sgo_wgs84_en(sgo_points(list(18.5, 54.2), epsg = 4326), to = 4936),
  fixed = TRUE,
...
# Check inputs
expect_error(
  sgo_en_wgs84(sgo_points(list(18.5, 54.2), epsg = 4326)),
  fixed = TRUE,
...
expect_true(all(
  abs(as.data.frame(p1[1:2]) - as.data.frame(p2[1:2])) < 0.000001
))
expect_equal(as.data.frame(p1[3:4]), as.data.frame(p2[3:4]))
