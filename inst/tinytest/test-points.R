...
#lists
expect_error(
       sgo_points(list(ln), epsg = 4326),
       "method accepts lists with at least 2 elements"
)
expect_error(
       sgo_points(list(-4.22472, 57.47777)),
       "'epsg' must be entered as one of the accepted numbers"
)
expect_error(
       sgo_points(list(-4.22472, 57.47777), epsg = -1),
       "'epsg' must be entered as one of the accepted numbers"
)
expect_error(
       sgo_points(
...
expect_error(
       sgo_points(df, epsg = -1),
       "'epsg' must be entered as one of the accepted numbers"
)

df <- data.frame(n, ln, stringsAsFactors = FALSE)
expect_error(sgo_points(df, epsg = 4326), "coordinates must be numeric")
...
expect_error(
       sgo_points(list(n = n, ln = ln, lt = lt), epsg = 4326),
       "Must specify the coordinate columns using the 'coords' parameter"
)

#all coords must be in x
expect_error(
       sgo_points(list(lon = ln, lt = lt), coords = c("ln", "lt"), epsg = 4326),
...
expect_error(
       sgo_points(
              list(x = -1.6644422222, y = 53.6119903611, z = 299.800),
              coords = c("x", "y"),
              epsg = 4937
       ),
...
expect_error(
       sgo_points(list(ln, lt.t), epsg = 4326),
       "All coordinates must be numeric"
)
...
expect_true(
       sgo_points(m, coords = c("ln", "lt", "h"), epsg = 4326)$epsg == 4979
)
expect_true(
       sgo_points(
              list(x = -1.6644422222, y = 53.6119903611, z = 299.800),
...
expect_warning(
       sgo_points(
              list(ln = ln, lt = lt, epsg = c(4326, 4326)),
              coords = c("ln", "lt"),
              epsg = 4326
       ),
       paste(
              "The column\\(s\\) from input data named epsg",
...
expect_warning(
       ...
       paste(
              "The column\\(s\\) from input data named epsg, dimension",
              "has\\(have\\) been renamed appending the suffix '.1'"
       )
)
...
expect_equal(
       colnames(sgo_coordinates(p, names.xyz = too.many.names)),
       new.names
)
...
expect_equal(
       sgo_coordinates(p, as.latlon = TRUE),
       matrix(
              data = c(lat, lon),
              ncol = 2,
              byrow = FALSE,
...
expect_equal(
       sgo_coordinates(p, names.xyz = new.names, as.latlon = TRUE),
       matrix(
              data = c(lat, lon),
              ncol = 2,
              byrow = FALSE,
...
expect_equal(
       sgo_coordinates(p, ll.format = "DMS"),
       matrix(
              data = c(
                     "4\U00B0 15\U2032 6.52\U2033 W",
                     "3\U00B0 11\U2032 17.77\U2033 W",
