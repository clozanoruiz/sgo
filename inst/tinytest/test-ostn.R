...
sgo.datum.flags <- sgo:::datum.flags
...
points.out <-
  as.data.frame(sgo_lonlat_bng(points.in, OSTN = TRUE)[c("PointID", "x", "y")])
pointsT.out <-
  as.data.frame(sgo_transform(points.in, to = 27700)[c("PointID", "x", "y")])
...
### 3D OSTN15_OSGM15_TestInput_ETRStoOSGB.txt ###
...
pointsgf.out <- as.data.frame(
  sgo_lonlat_bng(points.in, to = 7405, OSTN = TRUE, OD = TRUE),
  stringsAsFactors = FALSE
)
points.out <- as.data.frame(
  sgo_lonlat_bng(points.in, to = 7405, OSTN = TRUE),
  stringsAsFactors = FALSE
)
pointsTgf.out <- as.data.frame(
  sgo_transform(points.in, to = 7405, OSTN = TRUE, OD = TRUE),
  stringsAsFactors = FALSE
)
pointsT.out <- as.data.frame(
  sgo_transform(points.in, to = 7405, OSTN = TRUE, OD = FALSE),
  stringsAsFactors = FALSE
)

file.out <- file.path(getwd(), "OSTN15_OSGM15_TestOutput_ETRStoOSGB.txt")
ostn.out <- read.csv(file.out, stringsAsFactors = FALSE)[c(
  "OSGBEast",
  "OSGBNorth",
  "ODNHeight",
  "OSGBDatumFlag",
...
ostn.out[, "OSGBDatumFlag"] <- sgo.datum.flags[
  match(ostn.out[, "OSGBDatumFlag"], sgo.datum.flags$geoid.datum.flag),
  4
]
...
points.out <- as.data.frame(sgo_bng_lonlat(points.in, to = 4258, OSTN = TRUE)[c(
  "PointID",
  "x",
  "y"
)])
pointsT.out <-
  as.data.frame(sgo_transform(points.in, to = 4258)[c("PointID", "x", "y")])

file.out <- file.path(getwd(), "OSTN15_OSGM15_TestOutput_OSGBtoETRS.txt")
ostn.out <- read.csv(file.out, stringsAsFactors = FALSE)
ostn.out <- ostn.out[
  ostn.out$Iteration.No..RESULT == "RESULT",
...
expect_true(all(sweep(
  abs(points.out[, 2:3] - ostn.out[, 2:3]),
  2,
  threshold,
  "<"
)))
expect_true(all(sweep(
  abs(pointsT.out[, 2:3] - ostn.out[, 2:3]),
  2,
  threshold,
  "<"
)))
...
### 3D OSTN15_OSGM15_TestInput_OSGBtoETRS.txt ###

file.in <- file.path(getwd(), "OSTN15_OSGM15_TestInput_OSGBtoETRS.txt")
ostn.in <- read.csv(file.in, stringsAsFactors = FALSE)
points.in <- sgo_points(
  ostn.in,
  coords = c("OSGB36.Eastings", "OSGB36.Northing", "Ortho.Height"),
  epsg = 7405
)

pointsgf.out <- as.data.frame(
  sgo_bng_lonlat(points.in, to = 4937, OSTN = TRUE, OD = TRUE),
  stringsAsFactors = FALSE
)
points.out <- as.data.frame(
  sgo_bng_lonlat(points.in, to = 4937, OSTN = TRUE),
  stringsAsFactors = FALSE
)
pointsTgf.out <- as.data.frame(
  sgo_transform(points.in, to = 4937, OSTN = TRUE, OD = TRUE),
  stringsAsFactors = FALSE
)
pointsT.out <- as.data.frame(
  sgo_transform(points.in, to = 4937, OD = FALSE),
  stringsAsFactors = FALSE
)
...
ostn.out <- ostn.out[
  ostn.out$Iteration.No..RESULT == "RESULT",
  c("ETRSNorth.Long", "ETRSEast.Lat", "ETRSHeight", "OSGBDatumFlag", "PointID")
]
...
#coordinates
expect_true(all(sweep(
  abs(pointsgf.out[, 1:3] - ostn.out[, 1:3]),
  2,
  threshold.3d,
  "<"
)))
expect_true(all(sweep(
  abs(points.out[, 1:3] - ostn.out[, 1:3]),
  2,
  threshold.3d,
  "<"
)))
expect_true(all(sweep(
  abs(pointsTgf.out[, 1:3] - ostn.out[, 1:3]),
  2,
  threshold.3d,
  "<"
)))
expect_true(all(sweep(
  abs(pointsT.out[, 1:3] - ostn.out[, 1:3]),
  2,
  threshold.3d,
  "<"
)))
...
lat <- c(
  49.96006137820, 50.43885825610, 50.57563665000 ,50.93127937910,
...
lon <- c(
  -5.20304609998, -4.10864563561, -1.29782277240, -1.45051433700,
...
expect_true(all(
  abs(
    sgo_coordinates(sgo_bng_lonlat(sgo_points(
      list(91492.146, 11318.804),
      epsg = 27700
    ))) -
      c(-6.29977752014, 49.92226393730)
  ) < threshold
))

#List of points
array_of_xy <- sgo_coordinates(sgo_bng_lonlat(sgo_points(
  list(e, n),
  epsg = 27700
)))
...
expect_true(all(
  abs(
    sgo_coordinates(sgo_lonlat_bng(sgo_points(
      list(-6.29977752014, 49.92226393730),
      epsg = 4258
    ))) -
      c(91492.146, 11318.804)
  ) < 0.001
))

#List of points
array_of_xy <- sgo_coordinates(sgo_lonlat_bng(sgo_points(
  list(lon, lat),
  epsg = 4258
)))
...
expect_true(all(
  sgo_coordinates(
    sgo_lonlat_bng(sgo_points(list(0, 51.4775), epsg = 4277))
  ) == gw
))
expect_true(all(
  sgo_coordinates(
    sgo_lonlat_bng(sgo_points(list(-0.0015938564, 51.4780161357), epsg = 4326))
  ) == gw
))
expect_true(all(
  sgo_coordinates(
    sgo_lonlat_bng(
      sgo_points(list(-0.0015938564, 51.4780161357), epsg = 4326),
      OSTN = FALSE
    )
  ) == c(538876, 177344)
))
...
expect_warning(
  OSTN.coords <- sgo_coordinates(
    sgo_lonlat_bng(sgo_points(list(-2, 49), epsg = 4258))
  ),
  "outside of the OSTN15 rectangle"
)
expect_true(all(
  OSTN.coords ==
    sgo_coordinates(sgo_lonlat_bng(
      sgo_points(list(-2, 49), epsg = 4258),
      OSTN = FALSE
    ))
))
...
expect_warning(
  OSTN.coords <- sgo_coordinates(
    sgo_lonlat_bng(sgo_points(list(-10, 55), epsg = 4258))
  ),
  "outside of the OSTN15 rectangle"
)
expect_true(all(
  OSTN.coords ==
    sgo_coordinates(sgo_lonlat_bng(
      sgo_points(list(-10, 55), epsg = 4258),
      OSTN = FALSE
    ))
))
#BNG to Lon/Lat (-10, 55):
expect_warning(
  sgo_bng_lonlat(sgo_points(list(-111093, 596584), epsg = 27700)),
  "outside of the OSTN15 rectangle"
)
#Upper limit reached
expect_warning(
  sgo_bng_lonlat(sgo_points(list(111093, 1250000), epsg = 27700)),
  "outside of the OSTN15 rectangle"
)
expect_warning(
  sgo_bng_lonlat(sgo_points(list(700000, 596584), epsg = 27700)),
  "outside of the OSTN15 rectangle"
)
...
expect_warning(
  OSTN.coords <- sgo_coordinates(
    sgo_lonlat_bng(sgo_points(list(40, 66), epsg = 4326))
  ),
  "outside of the OSTN15 rectangle"
)
expect_true(all(
  OSTN.coords ==
    sgo_coordinates(sgo_lonlat_bng(
      sgo_points(list(40, 66), epsg = 4326),
      OSTN = FALSE
    ))
))
...
expect_equal(
  sprintf(
    c("%.6g", "%.7g"),
    sgo_coordinates(sgo_bng_lonlat(
      sgo_points(list(400096, -100086), epsg = 27700),
      OSTN = FALSE
    ))
  ),
  c("-2", "49")
...
expect_equal(
  sprintf(
    c("%.6g", "%.7g"),
    sgo_coordinates(sgo_bng_lonlat(
      sgo_points(list(133985, 604172), epsg = 27700),
      OSTN = FALSE
    ))
  ),
  c("-6.18834", "55.25972")
...
### Boundaries of OSTN15 rectangle (edge and at sea) ###
#Edge one (should give a warning, as the OSTN webpage)
expect_warning(
  val <- sgo_coordinates(
    sgo_lonlat_bng(sgo_points(list(-7.5569, 49.76705), epsg = 4258))
  ),
  "outside of the OSTN15 rectangle"
)
...
expect_warning(
  val <- sprintf(
    c("%.5g", "%.7g"),
    sgo_coordinates(
      sgo_bng_lonlat(sgo_points(list(21, 26), epsg = 27700))
    )
  ),
  "outside of the OSTN15 rectangle"
)
...
expect_true(all(
  sgo_coordinates(
    sgo_lonlat_bng(sgo_bng_lonlat(sgo_points(
      list(449960, 567710),
      epsg = 27700
    )))
...
expect_true(all(
  sgo_coordinates(
    sgo_lonlat_bng(sgo_bng_lonlat(sgo_points(list(x, y), epsg = 27700)))
  ) ==
...
expect_true(all(
  sgo_coordinates(sgo_lonlat_bng(sgo_bng_lonlat(
    sgo_points(list(x, y), epsg = 27700)
  ))) == cbind(res.x, res.y)
))
...
expect_true(all(
  sgo_coordinates(sgo_lonlat_bng(sgo_bng_lonlat(
    sgo_ngr_bng("SW 540 170")
  ))) == c(154000.000, 17000.000)
))
expect_true(all(
  sgo_coordinates(sgo_lonlat_bng(sgo_bng_lonlat(
    sgo_ngr_bng("SW 910150")
  ))) == c(191000.000, 15000.000)
))


### Input and internal conversions checks in BNG routines ###
## sgo_lonlat_bng
expect_error(
  sgo_lonlat_bng(sgo_points(list(77360, 895710), epsg = 27700)),
...
#expect_warning(sgo_lonlat_bng(sgo_points(list(-5.003508, 56.79685),
#                                         epsg=4326),to=7405),
...
expect_error(
  sgo_bng_lonlat(
    sgo_points(list(651409.903, 313177.270), epsg = 27700),
    to = 3857
  ),
...
#Warning from 2D to 3D
#expect_warning(sgo_bng_lonlat(sgo_points(list(651409.903, 313177.270),
#                                         epsg=27700), to=4937),
#               "Converted from 2D to 3D thus heights default to 0")
