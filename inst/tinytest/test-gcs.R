...
expect_error(
  sgo_set_gcs(sgo_points(list(18.5, 54.2), epsg = 4326), to = 7405),
  "only transforms to Geodetic Coordinate Systems"
)

# input 'll', 'c'
expect_equal(
  sgo_coordinates(sgo_set_gcs(
...
expect_true(all(
  abs(
    sgo_coordinates(sgo_set_gcs(
      sgo_points(list(18.5, 54.2, 20), epsg = 4937),
      to = 4936
    )) -
...
expect_equal(
  sgo_coordinates(sgo_set_gcs(p, to = 4277)),
  matrix(c(-4.63335099411, 54.0865177713), ncol = 2, dimnames = dnames)
)
p <- sgo_points(list(-1.6644422222, 53.6119903611, 299.800), epsg = 4979)
expect_equal(
  sgo_coordinates(sgo_set_gcs(p, to = 4277)),
  matrix(c(-1.66292822467, 53.6117492333), ncol = 2, dimnames = dnames)
)
...
# wrong input
p <- sgo_points(
  list(x = 3737197.092, y = -302954.150, z = 5142476.100, attr = "attr1"),
  coords = c("x", "y", "z"),
  epsg = 7405
)
expect_error(sgo_cart_lonlat(p), "can only convert from epsg 4936, 4978")

# additional elements
p <- sgo_cart_lonlat(sgo_points(
  list(x = 3737197.092, y = -302954.150, z = 5142476.100, attr = "attr1"),
  coords = c("x", "y", "z"),
  epsg = 4936
))
