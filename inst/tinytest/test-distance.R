...
# by.element = TRUE
expect_equal(
      sgo_distance(p, grid.true.distance = FALSE, by.element = TRUE),
      c(0, 0, 0)
)
...
expect_equal(
      round(sgo_distance(p1, p2, which = "Harvesine"), 3),
      matrix(376.544)
)
expect_equal(
      round(sgo_distance(p1, p2, which = "Harvesine", by.element = TRUE), 3),
      376.544
)

expect_equal(round(sgo_distance(p1, p2, which = "Vicenty"), 3), matrix(377.658))
expect_equal(
      round(sgo_distance(p1, p2, which = "Vicenty", by.element = TRUE), 3),
      377.658
)
...
m1 <- matrix(unlist(p1[1:2], use.names = FALSE), ncol = 2, byrow = FALSE) /
      RAD.2.DEG
m2 <- matrix(unlist(p2[1:2], use.names = FALSE), ncol = 2, byrow = FALSE) /
      RAD.2.DEG
...
expect_equal(
      unlist(d.vicenty, use.names = FALSE) * RAD.2.DEG,
...
expect_warning(
      d.vicenty <- sgo:::.direct.vicenty.ellipsoid(
            p,
            500,
            45 / RAD.2.DEG,
            "WGS84",
...
stirling.sgo <- sgo_bng_lonlat(
      sgo_points(stirling.p.const, epsg = 27700),
      to = 4326
)
coords <- sgo_coordinates(stirling.sgo)
stir.shift.one <- sgo_points(
      rbind(coords[-1, ], coords[1, ]),
