...
.coordinates.names <- cbind(
     x = c("x", "lon", "longitude", "e", "easting"),
...
lonlat.ellipsoid <- data.frame(
     ...
     e2 = c(
          0.006694379990141316996137233540,
...
lonlat.datum <- data.frame(
     datum = c("OSGB36", "WGS84", "ETRS89"),
...
# Get the OSTN15 and OSGM15 Developer pack from Ordnance Survey
...
.ostn.shifts <- data.matrix(OSTN15[c(
...
save(
     ...
     .epsgs,
     .sgo_points.attr,
     .sgo_points.2d.coords,
     .sgo_points.3d.coords,
     .sgo_points.2d.core,
     .sgo_points.3d.core,
     .coordinates.names,
     lonlat.ellipsoid,
