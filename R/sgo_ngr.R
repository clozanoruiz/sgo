...
#' @export
sgo_ngr_bng <- function(x, col = NULL, check.only = FALSE) {
  UseMethod("sgo_ngr_bng")
}
...
sgo_ngr_bng.list <- function(x, col = NULL, check.only = FALSE) {
  ...
  structure(
    c(
      lst,
      epsg = 27700,
      datum = .epsgs[.epsgs$epsg == 27700, "datum"],
      dimension = "XY"
    ),
    class = "sgo_points"
  )
...
sgo_bng_ngr.sgo_points <- function(x, digits = 10) {
  ...
  if (digits == 0) {
    ...
    n.dec <- n - n.int
    e.pad <- paste0(
      .substr.r(paste0('000000', e.int), 6),
      ifelse(e.dec > 0, substring(as.character(round(e.dec, 3)), 2), "")
    )
    n.pad <- paste0(
      ifelse(n.int < 1e6, .substr.r(paste0('000000', n.int), 6), n.int),
  ...
  e100k <- trunc(e / 100000)
  n100k <- trunc(n / 100000)
  ...
  let.pair <- paste0(
    .ngr.LUT$letter[match(l1, .ngr.LUT$num)],
