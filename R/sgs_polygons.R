...
#'
#' @description
#' Creates an sf object containing a collection of polygons (planar surfaces
#' defined by one exterior boundary and zero or more interior boundaries) from a
#' list of coordinates or WKB fields. This object facilitates all the operations
#' with polygons in this package.
#'
#' @name sgs_polygons
#' @usage sgs_polygons(x, epsg = NULL, wkb = NULL, coords = NULL, by = NULL,
#' buffer = NULL)
#' @param x A dataframe.
#' @param epsg Numeric. Sets the \code{epsg} code of the coordinates
...
#' row of the column must contain a number because when grouping points with
#' \code{by} the routine will also aggregate buffer distances by the same
#' column.
#' \code{buffer} must be expressed in the same units as the coordinates (decimal
#' degrees or meters). Take into account that \code{sf} does not correctly
...
sgs_polygons.data.frame <- function (x, epsg=NULL, wkb=NULL, coords=NULL,
  ...
  if(is.null(wkb)) {
    ...
    if(is.null(by)) { #There will be only one polygon
      ...
      points <- sf::st_as_sf(x, coords=coords, crs=epsg,
      ...
      polygons <- by(x, list(x[, by]),
                     FUN=function(x, coords, crs) {
