...
test_that("Geodetic area", {
  ...
  expect_true(all(abs(sgs_area(sgs_points(list(lon, lat), epsg=4326)) -
  ...
  stirling.sgs <- sgs_bng_lonlat(sgs_points(stirling.p.const, epsg=27700),
                                 to=4326)
  expect_equal(round(sgs_area(stirling.sgs) / 10000, 4), 216400.2725) #in ha
  ...
  expect_equal(round(sgs_area(stirling.sgs, interpolate=30), 2),
...
test_that("Planar area", {
  # Wrong input
  expect_error(sgs_area(sgs_points(list(-344578.12, 7751020.42), epsg=3857)),
               "function doesn't support the input's EPSG")

  # Lots of vertices (Stirling Parliament Constituency)
  stirling.p.const <- readRDS(file="stirling.p.const.rds")
  stirling.sgs <- sgs_points(stirling.p.const, epsg=27700)
  expect_equal(round(sgs_area(stirling.sgs) / 10000, 3), 216359.129) #in ha
})
